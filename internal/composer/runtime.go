...
func (r *Runtime) deployService(ctx context.Context, meta AppMeta, svcName string, svc Service, spec *Spec, sourceDir, netName string) error {
	...
	binds, err := r.resolveBinds(svc.Volumes, meta.Name, spec)
	if err != nil {
...
func (r *Runtime) resolveBinds(mounts []string, appName string, spec *Spec) ([]string, error) {
	...
	for _, m := range mounts {
		...
		// Resolve relative paths to /apps/<name>/...
		if strings.HasPrefix(src, "./") {
			abs := filepath.Join(r.appsRoot, appName, src[2:])
